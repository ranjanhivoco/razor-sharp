...
export default function CardPopup(props) {
  ...
  const handleCreateCard = async (e) => {
    ...
    // formData.append("bannerFeatured", bannerFeatured);

    await axios({
      method: "POST",
      url: `${ApiUrl}/card-category/create`,
      data: formData,
      dataType: "jsonp",
      headers: authHeader(),
    })
            ...
            <Form onSubmit={handleCreateCard}>
              ...
              <Row className="mb-4">
                ...
                >
                  ...
                  >
                    <option value={category}>Select Card Category</option>
                    {categoryList?.map((e) => {
                      return (
                        <option key={e._id} value={e.subCategory}>
                          {e?.subCategory}
                        </option>
                      );
                    })}
