...
export default function InventoryList() {
    ...
    await hostedSellerProductAxios
      ...
      .then((response) => {
        if (response.status === 200) {
          setbestSeller(response?.data?.allbestSeller);
        }
      });
    setLoading(false);
  };
  ...
  const pageCount = Math.ceil(bestSeller?.length / itemsPerPage);
  const handlePageClick = (event) => {
    const newOffset = (event.selected * itemsPerPage) % bestSeller?.length;
    ...
    <>
      {loading ? (
        ...
        <PageLayout>
          ...
          <Col xl={12}>
            <CardLayout>
              <Breadcrumb title={"Best seller List"}>
