...
export default function InventoryList() {
    ...
    await hostedSellerProductAxios
      ...
      .then((response) => {
        if (response.status === 200) {
          setFeaturedProducts(response?.data?.allFeatured);
        }
      });
    setLoading(false);
  };
  ...
  const displayUsers = featuredProducts
  ...
  const pageCount = Math.ceil(featuredProducts?.length / itemsPerPage);
  const handlePageClick = (event) => {
    const newOffset =
      (event.selected * itemsPerPage) % featuredProducts?.length;
    ...
    <>
      {loading ? (
        ...
        <PageLayout>
          ...
          <Col xl={12}>
            <CardLayout>
              <Breadcrumb title={"Featured List"}>
