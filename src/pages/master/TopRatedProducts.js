...
export default function InventoryList() {
    ...
    await hostedSellerProductAxios
      ...
      .then((response) => {
        if (response.status === 200) {
          settopRatedProducts(response?.data?.alltopRated);
        }
      });
    setLoading(false);
  };
  ...
  const displayUsers = topRatedProducts
  ...
  const pageCount = Math.ceil(topRatedProducts?.length / itemsPerPage);
  const handlePageClick = (event) => {
    const newOffset =
      (event.selected * itemsPerPage) % topRatedProducts?.length;
    ...
    <>
      {loading ? (
        ...
        <PageLayout>
          ...
          <Col xl={12}>
            <CardLayout>
              <Breadcrumb title={"Top Rated List"}>
