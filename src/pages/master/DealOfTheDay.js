...
export default function InventoryList() {
    ...
    await hostedSellerProductAxios
      ...
      .then((response) => {
        if (response.status === 200) {
          setdealOfTheDay(response?.data?.dealsOfTheDays);
        }
      });
    setLoading(false);
  };
  ...
  const displayUsers = dealOfTheDay
  ...
  const pageCount = Math.ceil(dealOfTheDay?.length / itemsPerPage);
  const handlePageClick = (event) => {
    const newOffset = (event.selected * itemsPerPage) % dealOfTheDay?.length;
    ...
    <>
      {loading ? (
        ...
        <PageLayout>
          ...
          <Col xl={12}>
            <CardLayout>
              <Breadcrumb title={"Deal of the Day List"}>
